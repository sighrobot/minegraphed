...
export const buildStats = (season, date) => {
  if (!date) return {}
...
export const buildStatsDiff = (season, toDate, fromDate) => {
  if (!toDate) return {}

  const currIdx = SESSIONS[season].indexOf(toDate)
  const prevIdx = fromDate ? SESSIONS[season].indexOf(fromDate) : currIdx + 1
...
export const customStats = (season, name, rawDate, isDiff = false) => {
  const date = rawDate ?? SESSIONS[season][0]
  const stats = (isDiff ? buildStatsDiff : buildStats)(season, date)
